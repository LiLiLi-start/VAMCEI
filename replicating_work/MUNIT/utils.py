...
from data import ImageFilelist, ImageFolder, ImageFolderTest
import torch
...
def get_data_loader_folder(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True, isTest=False, flip=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5),
                                           (0.5))]
    transform_list = [transforms.CenterCrop((height, width))] + transform_list if crop else transform_list
    # transform_list = [transforms.RandomHorizontalFlip()] + transform_list if flip else transform_list
    # transform_list = [transforms.RandomVerticalFlip()] + transform_list if flip else transform_list
    transform_list = [transforms.Resize(new_size)] + transform_list if new_size is not None else transform_list
    # transform_list = [transforms.RandomHorizontalFlip()] + transform_list if train else transform_list
    transform_list = [transforms.Grayscale(1)] + transform_list 
    transform = transforms.Compose(transform_list)
    if isTest:
        dataset = ImageFolderTest(input_folder, transform=transform)
    else:
