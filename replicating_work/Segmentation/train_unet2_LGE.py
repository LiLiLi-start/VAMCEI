...
import wandb
from evaluate import evaluate, evaluate_lge
...
dir_checkpoint = Path('./checkpoints_unet2_lge/')
...
def train_model(
    ...
    optimizer = optim.AdamW(model_2.parameters(),
                              lr=learning_rate, weight_decay=weight_decay, foreach=True)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'max', patience=5)  # goal: maximize Dice score
    grad_scaler = torch.cuda.amp.GradScaler(enabled=amp)
    criterion = nn.CrossEntropyLoss(weight=torch.Tensor([0.2, 0.25, 0.25, 0.3]).cuda()) if model.n_classes > 1 else nn.BCEWithLogitsLoss()
    ...
    for epoch in range(1, epochs + 1):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
                    ...
                    masks_pred_2 = model_2(torch.cat((masks_pred, images), dim=1))
                    ...
                    if model.n_classes == 1:
                        ...
                        #     multiclass=True
                        # )
                        edgeLoss2 = edge_loss(torch.argmax(masks_pred_2, dim=1, keepdim=True).float(), true_masks.unsqueeze(1).float())
                        loss = criterion(masks_pred_2, true_masks)
                        loss += edgeLoss2*0.5
                        
                        # loss += dice_loss(
                        #     F.softmax(masks_pred_2, dim=1).float(),
                        #     F.one_hot(true_masks, model.n_classes).permute(0, 3, 1, 2).float(),
                        #     multiclass=True
                        # )
                ...
                torch.nn.utils.clip_grad_norm_(model_2.parameters(), gradient_clipping)
        ...
        if save_checkpoint and val_score>val_score_max:
            ...
            torch.save(state_dict_2, str(dir_checkpoint / 'checkpoint_2_epoch{}.pth'.format(epoch)))
            torch.save(state_dict_2, str(dir_checkpoint / 'last_2.pth'))
...
if __name__ == '__main__':
    ...
    # if args.load:
    args.load = '/home/hfcui/Pytorch-UNet-master/checkpoints_unet1_lge/last.pth'
    state_dict = torch.load('/home/hfcui/Pytorch-UNet-master/checkpoints_unet1_lge/last.pth', map_location=device)
    # del state_dict['mask_values']
    model.load_state_dict(state_dict)
    logging.info(f'Model 1 loaded from {args.load}')
    
    args.load_2 = '/home/hfcui/Pytorch-UNet-master/checkpoints_unet2_c0/last.pth'
    state_dict = torch.load('/home/hfcui/Pytorch-UNet-master/checkpoints_unet2_c0/last.pth', map_location=device)
    # del state_dict['mask_values']
