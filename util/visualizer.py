...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256, use_wandb=False):
    ...
    # patient40_C0.nii.gz
    short_path = ntpath.basename(image_path['path'][0])
    name = os.path.splitext(short_path)[0]
    name += str(image_path['deep'].item())
    ...
    for label, im_data in visuals.items():
        ...
        # 将tensor转换为numpy.array 单通道灰度图像复制为三通道RGB图像
    ...
    def __init__(self, opt):
        ...
        if self.use_wandb:
            self.wandb_run = wandb.init(
                project='CycleGAN-and-pix2pix', name=opt.name, config=opt) if not wandb.run else wandb.run
